...
struct new_queue
{
    PCB *Parr;
    NQ *next;
} * Nque, *NTemp, *Rque, *point, *RUN;
int Sum_respone = 0, Sum_burst = 0, Sum_CPU = 0, Sum_ternaround = 0;
int time = 0, N = 10, MAX = 0, M = 0, itime=0;
FILE *input;
void NEW();
void Ready();
void terminate();
...
void Ready()
{
    ...
    else
    {
        if (RUN->Parr->priority < M)
        {
            ...
            {
                if (Rque->Parr->priority == M)
                {
                    RUN = Rque;
                    if (Rque->next->Parr->priority > NTemp->Parr->priority)
                    {
                        NTemp->next = Rque->next;
                        Rque = NTemp;
                    }
                    else
                    {
                        Rque = Rque->next;
                        point = Rque;
                        while (point->next != NULL && point->next->Parr->priority < NTemp->Parr->priority)
                            point = point->next;
                        NTemp->next = point->next;
                        point->next = NTemp;
                    }
                }
                else
                {
                    point = Rque;
                    while (point->next->Parr->priority != M)
                    {
                        point = point->next;
                    }
                    RUN = point->next;
                    if (RUN->next == NULL)
                        ...
                        point->next = RUN->next;
                    if (Rque->Parr->priority > NTemp->Parr->priority)
                    {
                        NTemp->next = Rque;
                        Rque = NTemp;
                    }
...
int start()
{
    ...
    fclose(input);
    printf("Scheduling : Preemptive Priority\n");
    printf("----------------------------------------------------\n");
    return 0;
}
...
void NEW()
{
    ...
    while (Npoints != NULL && i > 0)
    {
        ...
        {
            point = Rque;
            while (point->next != NULL && point->next->Parr->priority < Npoints->Parr->priority)
                point = point->next;
            TTemp = point->next;
            point->next = Npoints;
            point = point->next;
